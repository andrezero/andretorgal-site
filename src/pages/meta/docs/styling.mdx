...
**Note:** Because things are very responsive and fluid around here, make sure to also check [My website: Layout](/meta/docs/layout).

The `--layout-*` are semantic tokens for organising scale and rhythm in a responsive way:

- `--gap-v-legroom` - top padding for page layouts
- `--gap-v-headroom` - bottom padding for page layouts

For horizontal spacing:

- `--layout-unit` - minimum space between atoms
- `--gap-h-gutter` - on small screens
- `--gap-h-gutter-min`

For vertical spacing:

- `--layout-unit` - minimum space between atoms
- `--gap-h-s` - space between blocks
- `--gap-v-space` - between molecules
- `--gap-v-pause` - between page sections (organisms)

Along with some extra tokens for tweaking space proportionally, but with precision:

- `--layout-shift` and `--layout-unshift`
- `--proportional-quarter`, `--proportional-half`, `--proportional-one` - in relation to `1em`
- `--layout-border-thin` - outline-offsets and thin borders should not be scalable

<Note>

Although I am using atomic design nomenclature in some docs, I decided to drop these from the names of components, tokens, and file system groupings. See [Conventions: atomic semantics](/meta/records/rejected/conventions-structure-atomic-semantics) to know why.
