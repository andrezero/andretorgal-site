...
![Screenshot of the google image search results for "dragons": 15 thumbnails of colourful artwork are shown](./media/dragons.png 'There be dagons')
...
But if the reference to the image is no longer a string with a resolvable `path/to/image.png` then I can't reuse it easily - or can't use it at all - in other places such as the `og:image` tag, or in good old thumbnails in lists.
...
But the author is focused on other (much more important) things right now, and won't be able to help me with the [really strange and opaque CI issue](https://github.com/christian-hackyourshack/npm/issues/65) that I ran into. :red:

Time to take this into my own hands :fist: and have a go at putting together an "integration", using bits and pieces from all the things I saw in the last 2 weeks, and deliver that [improved (MDX) authoring experience in astro](/posts/)
